...
class Algolia_API
{
    ...
    public function get_client()
    {
        ...
        $integration_version = (string) apply_filters('algolia_ua_integration_version', ALGOLIA_VERSION . ' github:korben-info/wp-algolia');
        ...
        if (null === $this->client) {
            $this->client = Client::create($this->settings->get_application_id(), $this->settings->get_api_key());
        }
    ...
    {
        $client = Client::create((string) $application_id, (string) $api_key);
    ...
    {
        $client = Client::create((string) $application_id, (string) $search_api_key);
        try {
            // If this call does not succeed, then the application_ID or API_key is/are wrong.
